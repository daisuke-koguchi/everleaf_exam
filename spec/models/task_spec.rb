...
RSpec.describe 'タスクモデル機能', type: :model do
  describe 'validation' do
    ...
    it '優先順位がないとバリデーションに引っかかる'do 
      task = Task.new(name:'失敗テスト',priority:'')
      expect(task).not_to be_valid 
    end
    it 'タスクのタイトル、詳細、終了期限、ステータスがあるとバリデーションが通過する' do
      task = Task.new(name:'成功タイトル名',description:'成功詳細',deadline:'2000-01-01',status:'完了',priority:'高')
  ...
  describe '検索機能' do
    context 'scopeメソッドであいまい検索をした場合'do
      it '検索キーワードを含むタスクが絞り込まれる'do
        task = Task.create(name:'テスト1',description:'詳細1', deadline:'2000-01-01', status:'未着手', priority:'高')
        second_task = Task.create(name:"テスト2",description:'詳細2', deadline:'2000-01-01', status:'完了', priority:'高')
